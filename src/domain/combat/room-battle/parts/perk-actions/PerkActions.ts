import { FULL_BAR, PERK_BY_ID, type PerkDef } from '../../../../catalog';
...
/** Способности в бою: готовность, цена, заряд и наведение, применение через реестр. */
export class PerkActions extends RoomPart {
  ...
  usePerk(id: string): TurnResult {
    ...
    if (!this.state.stats.perks.includes(id))
      return { ok: false, reason: 'invalid', events: this.state.engine.flush() };
    if (this.state.armed?.id === id) {
      this.state.armed = null;
      this.state.swapFirst = null;
      this.state.emit({ type: 'armed', id: null });
    ...
    this.state.armed = p;
    this.state.swapFirst = null;
    this.state.emit({ type: 'armed', id: p.id });
    return { ok: true, events: this.state.engine.flush() };
  }
  ...
  /** Подходит ли карта под вид цели способности (`PerkDef.target`). */
  private inReach(p: PerkDef, card: Card, cell: CellIndex): boolean {
    switch (p.target) {
      case 'enemy':
