...
export class PhaserInput implements IInput {
  ...
  private static readonly STEPS: Record<string, [number, number]> = {
    UP: [0, -1], W: [0, -1], DOWN: [0, 1], S: [0, 1], LEFT: [-1, 0], A: [-1, 0], RIGHT: [1, 0], D: [1, 0],
  };

  private static readonly ITEM_KEYS: Record<string, ConsumableId> = { ONE: 'potion_heal', TWO: 'potion_regen', THREE: 'artifact' };
  ...
  constructor(scene: Phaser.Scene, private readonly playerCell: () => number) {
    ...
    for (const [key, [dx, dy]] of Object.entries(PhaserInput.STEPS)) kb?.on(`keydown-${key}`, () => this.step(dx, dy));
    for (const [key, itemId] of Object.entries(PhaserInput.ITEM_KEYS)) kb?.on(`keydown-${key}`, () => this.emit({ type: 'use-item', itemId }));
