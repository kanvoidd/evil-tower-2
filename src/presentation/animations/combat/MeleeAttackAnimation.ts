...
export class MeleeAttackAnimation implements IAttackAnimation {
  constructor(private readonly tweens: PhaserTweens, private readonly motion: BackgroundMotion) {}
  ...
  async play(view: CardView, targetCell: number): Promise<void> {
    ...
    view.c.setDepth(60);
    await this.tweens.play({ targets: view.c, x: to.x, y: to.y, duration: TIMING.attackLunge / 2, ease: 'Quad.easeIn' });
    this.motion.follow(
      this.tweens.play({ targets: view.c, x: from.x, y: from.y, duration: TIMING.attackLunge / 2, ease: 'Quad.easeOut' }).then(() => {
  ...
  rush(view: CardView, toward: Point): void {
    ...
    void this.tweens.play({
      targets: view.c, x: (home.x + toward.x) / 2, y: (home.y + toward.y) / 2, duration: 80, yoyo: true,
