...
import type { VfxStyle } from '../../../../domain/data/perks';
...
export class Vfx {
  ...
  play(style: VfxStyle, cells: Point[], from: Point): number {
    switch (style) {
      case 'bolt': cells.forEach((c) => this.lightning(from, c, Vfx.TINT.bolt, 3)); break;
      case 'chain': this.chain(from, cells); break;
      case 'arcane': cells.forEach((c) => this.arcane(c)); break;
      case 'beam': cells.forEach((c) => this.pillar(c, Vfx.TINT.beam, 74)); break;
  ...
  private glow(p: Point, color: number, size: number, ms = 320): void {
    const g = this.s.add.image(p.x, p.y, 'glow').setTint(color).setDisplaySize(size, size)
      .setBlendMode(Phaser.BlendModes.ADD).setDepth(Vfx.DEPTH).setAlpha(0.9);
  ...
  private ring(p: Point, color: number, to: number, ms = 400, width = 6): void {
    const r = this.s.add.image(p.x, p.y, 'ring').setTint(color).setDisplaySize(20, 20)
      .setDepth(Vfx.DEPTH).setAlpha(0.95).setBlendMode(Phaser.BlendModes.ADD);
    void width;
  ...
  private sparks(p: Point, color: number, n: number, spread = 70): void {
    for (let i = 0; i < n; i++) {
      const s = this.s.add.image(p.x, p.y, 'dot').setTint(color).setDepth(Vfx.DEPTH)
  ...
  private seg(a: Point, b: Point, color: number, w: number, alpha = 1): Phaser.GameObjects.Image {
    ...
    const img = this.s.add.image(a.x, a.y, 'px').setOrigin(0, 0.5).setTint(color)
      .setDisplaySize(len, w).setDepth(Vfx.DEPTH).setAlpha(alpha).setBlendMode(Phaser.BlendModes.ADD);
  ...
  private arcane(p: Point): void {
    const r = this.s.add.image(p.x, p.y, 'ring').setTint(Vfx.TINT.arcane).setDisplaySize(160, 160)
      .setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD);
    this.s.tweens.add({
  ...
  private pillar(p: Point, color: number, w: number): void {
    const beam = this.s.add.image(p.x, p.y, 'px').setTint(color).setDisplaySize(w, 0)
      .setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD).setAlpha(0.9);
    const core = this.s.add.image(p.x, p.y, 'px').setTint(0xffffff).setDisplaySize(w * 0.3, 0)
      .setDepth(Vfx.DEPTH + 1).setBlendMode(Phaser.BlendModes.ADD);
  ...
  private ignite(p: Point): void {
    const r = this.s.add.image(p.x, p.y, 'ring').setTint(Vfx.TINT.ignite).setDisplaySize(30, 30)
      .setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD);
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const e = this.s.add.image(x, p.y + 20, 'dot').setTint(i % 2 ? 0xffd15a : Vfx.TINT.ignite)
        .setDisplaySize(9, 9).setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD);
      this.s.tweens.add({
  ...
  private fireball(from: Point, to: Point): void {
    const ball = this.s.add.image(from.x, from.y, 'glow').setTint(Vfx.TINT.fireball)
      .setDisplaySize(52, 52).setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD);
    const core = this.s.add.image(from.x, from.y, 'dot').setTint(0xfff1c0)
      .setDisplaySize(22, 22).setDepth(Vfx.DEPTH + 1).setBlendMode(Phaser.BlendModes.ADD);
    const trail = this.s.time.addEvent({
      delay: 22,
      repeat: 9,
      callback: () => {
        const t = this.s.add.image(core.x, core.y, 'dot').setTint(Vfx.TINT.fire)
          .setDisplaySize(16, 16).setDepth(Vfx.DEPTH - 1).setBlendMode(Phaser.BlendModes.ADD);
  ...
  private inferno(cells: Point[]): void {
    const sheet = this.s.add.image(360, 640, 'px').setTint(Vfx.TINT.inferno).setDisplaySize(720, 1280)
      .setDepth(Vfx.DEPTH - 3).setAlpha(0).setBlendMode(Phaser.BlendModes.ADD);
  ...
  private corpse(p: Point): void {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const seg = this.seg(p, { x: p.x + Math.cos(a) * 80, y: p.y + Math.sin(a) * 80 }, Vfx.TINT.corpse, 7, 0.8);
  ...
  private ghost(from: Point, cells: Point[]): void {
    cells.forEach((c, i) => {
      const g = this.s.add.image(from.x, from.y, 'glow').setTint(Vfx.TINT.ghost)
        .setDisplaySize(44, 44).setDepth(Vfx.DEPTH).setAlpha(0.85).setBlendMode(Phaser.BlendModes.ADD);
      this.s.tweens.add({
  ...
  private voodoo(from: Point, to: Point): void {
    ...
    const r = this.s.add.image(to.x, to.y, 'ring').setTint(Vfx.TINT.voodoo).setDisplaySize(150, 150)
      .setDepth(Vfx.DEPTH).setAlpha(0);
    this.s.tweens.add({
  ...
  private harvest(to: Point, cells: Point[]): void {
    cells.forEach((c, i) => {
      const w = this.s.add.image(c.x, c.y, 'dot').setTint(Vfx.TINT.harvest)
        .setDisplaySize(18, 18).setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD);
      this.s.tweens.add({
    ...
    this.s.time.delayedCall(cells.length * 55 + 320, () => this.ring(to, Vfx.TINT.harvest, 260, 420));
  }
  ...
  private flames(p: Point): void {
    for (let i = 0; i < 7; i++) {
      ...
      const f = this.s.add.image(x, p.y + 40, 'flame').setTint(i % 2 ? 0xffd15a : Vfx.TINT.fire)
        .setDisplaySize(26, 40).setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD).setAlpha(0.95);
      this.s.tweens.add({
  ...
  private banner(cells: Point[]): void {
    ...
    const top = this.s.add.image(cx, 200, 'px').setTint(Vfx.TINT.banner).setDisplaySize(240, 0)
      .setDepth(Vfx.DEPTH).setAlpha(0.8).setOrigin(0.5, 0);
  ...
  private souls(p: Point): void {
    for (let i = 0; i < 5; i++) {
      const w = this.s.add.image(p.x + (Math.random() - 0.5) * 70, p.y, 'dot').setTint(Vfx.TINT.soul)
        .setDisplaySize(14, 14).setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD);
      this.s.tweens.add({
  ...
  private mark(p: Point): void {
    const r = this.s.add.image(p.x, p.y, 'ring').setTint(Vfx.TINT.mark).setDisplaySize(190, 190)
      .setDepth(Vfx.DEPTH).setAlpha(0);
    this.s.tweens.add({
  ...
  private arrow(from: Point, to: Point): void {
    ...
    const shaft = this.s.add.image(from.x, from.y, 'arrow_vfx').setDepth(Vfx.DEPTH)
  ...
  private fallingArrow(to: Point, delay: number): void {
    const a = this.s.add.image(to.x, to.y - 320, 'arrow_vfx').setDepth(Vfx.DEPTH)
  ...
  private swap(cells: Point[]): void {
    for (const p of cells) {
      const r = this.s.add.image(p.x, p.y, 'ring').setTint(Vfx.TINT.swap).setDisplaySize(40, 40)
        .setDepth(Vfx.DEPTH).setBlendMode(Phaser.BlendModes.ADD);
  ...
  private rewind(p: Point): void {
    for (let i = 0; i < 3; i++) {
      const r = this.s.add.image(p.x, p.y, 'ring').setTint(Vfx.TINT.rewind).setDisplaySize(260, 260)
        .setDepth(Vfx.DEPTH).setAlpha(0.9).setBlendMode(Phaser.BlendModes.ADD);
      this.s.tweens.add({
