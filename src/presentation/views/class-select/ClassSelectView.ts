...
import type { Trait, TraitId } from '../../../domain/logic/traits';
import type { ClassId, LineageId } from '../../../domain/types';
import { describeTrait, t, type TKey } from '../../../i18n';
import {
  addEmbers, background, closeButton, icon, PlateButton, plateTexture, shadowTexture, staggerIn, statChip, statDisc, toast,
  txt, UiSound,
} from '../../components';
...
export class ClassSelectView implements IClassSelectView {
  ...
  private static readonly TRAIT_STAT: Partial<Record<TraitId, string>> = {
    crit: 'crit', dodge: 'dodge', parry: 'parry', armor: 'defense', dmg: 'damage', hp: 'health',
  };
  ...
  constructor(private readonly scene: Phaser.Scene, private readonly d: ClassSelectViewDeps) {
    const mode = d.selection.mode;
    background(scene);
    addEmbers(scene, 10);
    dollyIn(scene, 380);

    const title = txt(scene, GAME_W / 2, 92, mode === 'first' ? t('select.first') : t('select.title'), 48, { font: 'title', color: HEX.gold, strokeThickness: 6 });
    ...
    this.nameText = txt(scene, 0, -212, '', 46, { font: 'title', color: HEX.gold, maxWidth: w - 60, strokeThickness: 0 });
    this.info.add(this.nameText);
    // у каждого героя свой рекорд забега и свой кошелёк — рекорд видно прямо в выборе героя
    this.climbText = txt(scene, 0, -174, '', 21, { color: HEX.textDim, weight: 800, strokeThickness: 0 });
    this.info.add(this.climbText);
    this.info.add(scene.add.image(0, -172, 'px').setTint(0xffffff).setAlpha(0.1).setDisplaySize(w - 80, 2));
    ...
    this.info.add(scene.add.image(0, -8, 'px').setTint(0xffffff).setAlpha(0.1).setDisplaySize(w - 80, 2));
    ...
    this.buildArrows();
    scene.tweens.add({ targets: this.pulseK, v: CAROUSEL.pulseScale, duration: CAROUSEL.pulseCycle / 2, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
    this.bindInput();
    ...
    const hint = txt(scene, GAME_W / 2, 1238, t('select.hint'), 22, { color: HEX.textMute, weight: 700, strokeThickness: 0 });
    this.layout();
  ...
  private tapAt(x: number, y: number): void {
    ...
    this.items.forEach((it, i) => {
      ...
      if (dist < bestD && this.wrap(i - this.pos) !== 0 && Math.abs(this.wrap(i - this.pos)) < 1.6) {
        best = i;
        bestD = dist;
      }
    });
  ...
  private buildArrows(): void {
    const mk = (x: number, dir: -1 | 1): void => {
      const b = new PlateButton(this.scene, x, ClassSelectView.CY + 40, { w: 60, h: 60, icon: 'svg_chevron', iconSize: 24, radius: 30, onClick: () => this.step(dir) });
  ...
  private buildItem(classId: ClassId, lineage: LineageId, opened: boolean): CarouselItem {
    ...
    const glow = s.add.image(0, 0, 'glow').setTint(col).setDisplaySize(520, 520).setBlendMode(Phaser.BlendModes.ADD);
    const pic = s.add.image(0, 0, `cls_${classId}`).setDisplaySize(300, 300);
    const lock = s.add.container(0, 0);
    lock.add([s.add.image(0, 0, plateTexture(s, 116, 116, 1, 'raised', 58)), icon(s, 0, 0, 'svgw_lock', 60)]);
  ...
  private refreshInfo(first: boolean): void {
    ...
    list.forEach((tr, i) => {
      ...
      this.traits.add(statDisc(s, -w / 2 + 52, y, vis.stat, 28, { color: vis.color, iconKey: vis.iconKey }));
    ...
    [['health', String(st.maxHp)], ['damage', String(st.damage)], ['crit', `${Math.round(st.crit)}%`]].forEach(([stat, val], i) => {
    ...
    if (choice.action === 'start') this.action.setLabel(t('select.start')).setStyle('gold').setLocked(false);
    else if (choice.action === 'unlock') this.action.setLabel(t('select.unlock', { n: this.d.selection.unlockCost })).setStyle('gold').setLocked(false);
    else if (choice.action === 'current') this.action.setLabel(t('select.current')).setStyle('raised').setLocked(true);
    else this.action.setLabel(t('select.pick')).setStyle('gold').setLocked(false);
  }

  private static traitVisual(tr: Trait, lineageColor: string): { stat: string; color: number; iconKey: string } {
