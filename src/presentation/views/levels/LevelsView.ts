...
import { FLOORS } from '../../../domain/data/floors';
import { ROOMS, ROOMS_PER_FLOOR } from '../../../domain/data/levels';
import { t, type TKey } from '../../../i18n';
import {
  background, closeButton, PlateButton, plateTexture, ScrollList, shadowTexture, staggerIn, toast, txt, UiSound,
} from '../../components';
...
export class LevelsView {
  ...
  constructor(private readonly scene: Phaser.Scene, private readonly d: LevelsViewDeps) {
    const L = LevelsView;
    const s = scene;
    background(s);
    const title = txt(s, GAME_W / 2, 66, t('levels.title'), 54, { font: 'title', color: HEX.gold, strokeThickness: 6 });
    const record = txt(s, GAME_W / 2, 106, t('levels.record', { n: d.record.best, max: ROOMS.length }), 20, { color: HEX.textDim, weight: 800, strokeThickness: 0 });
    ...
    for (let f = FLOORS; f >= 1; f--) {
      ...
      c.add(s.add.image(cx, yTop + slabH / 2 + 8, shadowTexture(s, L.SLAB_W, slabH, 32, 20)).setAlpha(0.85));
      ...
      if (f === FLOORS) {
        ...
        for (let n = 0; n < teeth; n++) {
          c.add(s.add.image(l + n * 2 * tw + tw / 2, yTop - 8, plateTexture(s, tw, 36, 1, 'panel', 8)));
        }
      }

      // заголовок этажа: название слева, прогресс справа
      const done = ROOMS.filter((rm, i) => rm.floor === f && this.reached(i) === 'cleared').length;
      c.add(txt(s, l + 34, yTop + 40, `${t('levels.floor', { n: f })} · ${t(`floor.${f}.name` as TKey)}`, 30, { font: 'title', origin: [0, 0.5], color: HEX.gold, strokeThickness: 0 }));
      c.add(txt(s, r - 34, yTop + 40, `${done} / ${ROOMS_PER_FLOOR}`, 24, { origin: [1, 0.5], color: done === ROOMS_PER_FLOOR ? HEX.good : HEX.textDim, weight: 900, strokeThickness: 0 }));
  ...
  private roomButton(id: string, state: 'cleared' | 'frontier' | 'locked', x: number, y: number, w: number, boss: boolean): Phaser.GameObjects.Container {
    const s = this.scene;
    ...
    if (frontier) {
      const glow = s.add.image(0, 0, 'glow').setTint(COLOR.gold).setBlendMode(Phaser.BlendModes.ADD).setDisplaySize(w + 90, LevelsView.TILE_H + 90).setAlpha(0.3);
      s.tweens.add({ targets: glow, alpha: { from: 0.12, to: 0.36 }, duration: 1700, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
      holder.add(glow);
    }
    const style = frontier ? 'gold' : cleared ? 'green' : 'dark';
    const iconKey = !available ? 'svg_lock' : boss ? 'svg_skull' : cleared ? 'svg_check' : undefined;
    const btn = new PlateButton(s, 0, 0, {
      w, h: LevelsView.TILE_H, label: available ? id : undefined, fontSize: boss ? 40 : 36, radius: 26, style,
      icon: iconKey, iconSize: !available ? 40 : 30, sound: null, shadow: available,
      ...
      onClick: () => {
        ...
        toast(s, available ? t('levels.run_hint') : t('levels.locked'), available ? 'svg_arrow' : 'svg_lock');
      },
    });
