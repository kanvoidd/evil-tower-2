...
import { closeButton, CurrencyBar, icon, PlateButton, tipOnHover, txt, type IWalletSource } from '../../components';
...
export class SkillTreeHeader {
  ...
  constructor(
    ...
    const q = query;
    scene.add.rectangle(GAME_W / 2, 66, GAME_W, 132, 0x0a0c12, 0.9).setScrollFactor(0).setDepth(700);
    scene.add.rectangle(GAME_W / 2, 132, GAME_W, 2, 0xffffff, 0.06).setScrollFactor(0).setDepth(700);
    this.classBtn = new PlateButton(scene, 66, 66, {
      w: 84, h: 84, icon: `cls_${q.activeClass}`, iconSize: 68, radius: 26, style: 'gold',
      onClick: () => commands({ type: 'switch-class' }),
    });
    this.classBtn.setScrollFactor(0).setDepth(710);
    const swap = scene.add.container(66 + 30, 66 + 30).setScrollFactor(0).setDepth(712);
    swap.add([scene.add.circle(0, 0, 15, 0x0b0d12).setStrokeStyle(2, 0xf0c75e), icon(scene, 0, 0, 'svgw_swap', 18)]);
    const name = txt(scene, 128, 52, t(`class.${q.activeClass}.name` as TKey), 32, { font: 'title', origin: [0, 0.5], color: HEX.gold, maxWidth: 210, strokeThickness: 5 })
    ...
    const cur = new CurrencyBar(scene, 590, 44, { wallet, compact: true }).setScrollFactor(0).setDepth(710);
    ...
    this.autoBtn = new PlateButton(scene, 384, 66, {
      w: 68, h: 68, icon: 'svg_auto', iconSize: 34, radius: 22, style: 'raised', onClick: () => commands({ type: 'toggle-auto' }),
    });
    ...
    tipOnHover(scene, this.autoBtn, () => `${t('auto.skill.tip')}\n${t(q.autoSkillOn ? 'auto.state.on' : 'auto.state.off')}`);
