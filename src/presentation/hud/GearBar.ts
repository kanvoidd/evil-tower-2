...
export class GearBar {
  ...
  update(weapon: EquipmentSave | null, armor: EquipmentSave | null): void {
    ...
    gear.forEach(({ e, x }) => {
      ...
      this.row.add(txt(s, x + 16, y + 46, `${e.durability}/${it.durability}`, 16, { weight: 800, strokeThickness: 0, color: k <= 0.2 ? HEX.bad : HEX.textDim }));
      this.row.add(s.add.rectangle(x + 16, y + 64, 84, 6, 0x000000, 0.5));
      this.row.add(s.add.rectangle(x - 26, y + 64, Math.max(3, 84 * k), 6, k > 0.2 ? 0x66e39c : 0xff7468).setOrigin(0, 0.5));
    });
  }
}
