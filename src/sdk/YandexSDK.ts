...
class YandexSdkWrapper {
  ...
  onGamePause(h: PauseHandler): void {
    this.onPause.push(h);
  }

  onGameResume(h: PauseHandler): void {
  ...
  showInterstitial(): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      if (this.isMock) {
        mockAdOverlay('Реклама (заглушка)', false).then(() => done(true));
        return;
      }
  ...
  showRewarded(): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      if (this.isMock) {
        mockAdOverlay('Видео с наградой (заглушка)', true).then((ok) => {
          rewarded = ok;
          done();
        });
        return;
  ...
  async askReview(): Promise<void> {
    if (this.isMock || !this.ysdk?.feedback) return;
    try {
      const { canReview } = await this.ysdk.feedback.canReview();
      if (canReview) await this.ysdk.feedback.requestReview();
    } catch {
      /* noop */
    }
  }
...
function mockAdOverlay(title: string, rewarded: boolean): Promise<boolean> {
  return new Promise((resolve) => {
...
export const YSDK = new YandexSdkWrapper();
