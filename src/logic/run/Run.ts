...
export class Run implements IRunSession {
  ...
  private archerReach(cell: number): boolean {
    ...
    if (Grid.dist(p, cell) === 2 && Grid.sameLine(p, cell)) return true;
    if (this.stats.passives.has('diagonal') && Grid.diagonals(p).includes(cell)) return true;
    if (this.stats.passives.has('eagle_eye') && Grid.neighbors(p).includes(cell)) return true;
    return false;
  }
  ...
  private basicRanged(cell: number): void {
    ...
    this.emit({ type: 'attack', from: this.playerCell, to: cell, ranged: true, by: 'player', style });
    this.wearWeapon();
