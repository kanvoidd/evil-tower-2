...
| Прогресс героя | `progression/` | герой и его класс (`hero/`), дерево прокачки (`skill-tree/`: узлы, состояние, покупка, изученное — без знания эффектов), сборка характеристик (`stats/`, что дают изученные таланты — `stats/talent-bonuses/`), цены в душах (`soul-prices/`: `ISoulPricing` и реализация по таблице ступеней), автопрокачка (`auto-skill/`), сводка класса (`traits/`) | герой, класс, метаморфоза, талант, ярус, путь |
