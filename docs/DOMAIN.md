...
| Каталог | `catalog/` | содержимое игры без текстов (названия и описания — в словарях `src/i18n/` по id сущности), содержимое героев — определения в папке линейки (`heroes/<линейка>/`: `lineage.ts`, классы с перками по слотам `classes/`, способности `abilities/<класс>.ts`), таланты пока выпускают фабрики героев (`HeroFactory`, до этапа D `docs/REFACTORING-PLAN-3.md`); виды и реестры способностей — `abilities/`, перков — `perks/`, этажи, враги и комнаты (`floors/`, `enemies/`, `levels/`, `FloorFactory`) и рост силы и наград от этажа к этажу (`floors/scaling/`: `IFloorScaling`), вещи (`items/`, `weapons/`, `armor/`), расходники (`consumables/`) | линейка, класс, способность, талант, этаж, враг, комната, вещь, расходник |
...
| `catalog` | реестры и фабрики содержимого (`HEROES`, `LINEAGES`, `HERO_FACTORIES`, `CLASSES`, `CLASS_DEFINITIONS`, `ABILITY_LIST`, `PERKS`, `TALENTS`, `FLOORS`, `FLOOR_FACTORIES`, `ENEMIES`, `ROOMS`, `MODIFIERS`, `ITEMS`, `WEAPONS`, `ARMORS`, `CONSUMABLES`) и их поиск (`perkOf`, `ABILITY_BY_ID`, `withBehavior`, `rollRoom`, `ITEM_BY_ID` …), масштабирование этажей (`IFloorScaling`, `FLOOR_SCALING`, `FloorCurveScaling`), содержимое линейки `HeroContent`, id содержимого (`LineageId`, `ClassId`, `ConsumableId`, `TalentPath`, `CardKind` …), надетая вещь `EquipmentSave`, типы определений (`LineageDef`, `ClassDefinition`, `AbilityDef`, `PerkDef`, `AbilityBehaviorId`, `TalentDef`, `EnemyDef`, `RoomDef`, `ItemDef` …) |
